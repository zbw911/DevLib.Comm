...
namespace H.BLL
{
    public class UserView
    {
        ...
        /// <param name="dt"></param>
        /// <param name="toid"></param>
        public static void TrackeUserAction(string userid, string url, string referer, string Agent, string IP, DateTime dt)
        {
            if (!Config.IsUseHbase) return;
            ...
            using (var hclient = HBaseClientPool.GetHclient())
            {
                ...
                hclient.Client.mutateRow(tableName.ToBytes(), rowkey.ToBytes(),
                    ...
                    new Apache.Hadoop.Hbase.Mutation{ Column= "t:dt".ToBytes(), Value = dt.ToString().ToBytes() },
                    new Apache.Hadoop.Hbase.Mutation{ Column= "t:ref".ToBytes(), Value = referer.ToBytes() },
                    new Apache.Hadoop.Hbase.Mutation{ Column= "t:agt".ToBytes(), Value = Agent.ToBytes() },
                    new Apache.Hadoop.Hbase.Mutation{ Column= "t:ip".ToBytes(), Value = IP.ToBytes() } 
                });
            }
        }
    }
